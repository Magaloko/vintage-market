...
export default function CompareBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 backdrop-blur-md shadow-2xl"
      style={{ backgroundColor: 'rgba(14, 26, 43, 0.95)', borderTop: '1px solid rgba(176, 141, 87, 0.3)' }}>
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-4">
        <GitCompareArrows size={20} style={{ color: '#B08D57' }} className="shrink-0" />
        <div className="flex items-center gap-3 flex-1 overflow-x-auto">
          {compareItems.map(item => (
            <div key={item.id} className="flex items-center gap-2 rounded-full pl-1 pr-3 py-1 shrink-0"
              ...
              <span className="font-sans text-xs max-w-[100px] truncate" style={{ color: 'rgba(240, 230, 214, 0.8)' }}>{item.title}</span>
              <button onClick={() => removeFromCompare(item.id)} style={{ color: 'rgba(240, 230, 214, 0.4)' }}
